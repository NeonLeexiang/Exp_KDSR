...
class FSRCNN(nn.Module):
   def __init__(self, scale, n_colors, d=56, s=12, m=4, fsrcnn_weight_init=False):
       ...
       self.feature_extraction.append(nn.Sequential(
           Conv2d(in_channels=n_colors, out_channels=d, kernel_size=5, stride=1, padding=2),
           nn.PReLU()))
       self.shrinking = []
       self.shrinking.append(nn.Sequential(
           Conv2d(in_channels=d, out_channels=s, kernel_size=1, stride=1, padding=0),
           nn.PReLU()))
       self.mapping = []
       for _ in range(m):
           self.mapping.append(nn.Sequential(
               Conv2d(in_channels=s, out_channels=s, kernel_size=3, stride=1, padding=1),
               nn.PReLU()))
       self.expanding = []
       self.expanding.append(nn.Sequential(
           Conv2d(in_channels=s, out_channels=d, kernel_size=1, stride=1, padding=0),
           nn.PReLU()))
       self.last_layer = []
...
class FSRCNNAutoencoder(FSRCNN):
    def __init__(self, scale, n_colors, d=56, s=12, m=4, k=1, encoder='inv_fsrcnn'):
        ...
        self.feature_extraction = []
        self.feature_extraction.append(nn.Sequential(
            Conv2d(in_channels=n_colors*k, out_channels=d, kernel_size=5, stride=1, padding=2),
            nn.PReLU()))
        self.network = nn.Sequential(
            OrderedDict([
...
def get_fsrcnn_addernet_teacher(scale, n_colors, **kwargs):
    return FSRCNNTeacher(scale, n_colors, **kwargs)


def get_fsrcnn_addernet_student(scale, n_colors, **kwargs):
